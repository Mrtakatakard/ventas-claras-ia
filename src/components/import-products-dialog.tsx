...
import { batchAddProducts } from '@/lib/firebase/service'
...
export function ImportProductsDialog({
  ...
  const handleSaveProducts = async () => {
    ...
    try {
        ...
        await batchAddProducts(productsToSave, userId, existingCategories);
  ...
  const handleSelectAll = (checked: boolean | 'indeterminate') => {
    setSelectedRows(checked ? new Set(validatedProducts.map(p => p.rowIndex)) : new Set());
  };
  ...
  const handleApplyBulkCategory = () => {
    ...
    const categoryName = existingCategories.find(c => c.id === bulkCategoryId)?.name;
  ...
  const hasErrors = useMemo(() => validatedProducts.some(p => p.status === 'error'), [validatedProducts]);
  const hasMissingCurrency = useMemo(() => validatedProducts.some(p => !p.currency), [validatedProducts]);
  const hasMissingPrice = useMemo(() => validatedProducts.some(p => p.batches.some(b => b.price <= 0)), [validatedProducts]);
        ...
        <div className="max-h-[60vh] overflow-auto border rounded-md">
          <TooltipProvider>
            <Table>
              ...
              <TableBody>
                {validatedProducts.map((product, productIndex) => (
                        ...
                        <TableRow key={`${product.rowIndex}-${batchIndex}`} className={cn(product.status === 'error' && 'bg-destructive/10')}>
                          {batchIndex === 0 && (
                            <TableCell rowSpan={product.batches.length} className="align-top pt-3">
                              <Checkbox checked={selectedRows.has(product.rowIndex)} onCheckedChange={(checked) => handleRowSelect(product.rowIndex, !!checked)} />
                            </TableCell>
                          )}
                          {batchIndex === 0 && (
                            <TableCell rowSpan={product.batches.length} className="align-top pt-3">
                          ...
                          {batchIndex === 0 && <TableCell rowSpan={product.batches.length} className="align-top pt-2 font-mono"><Input value={product.code} onChange={e => handleFieldChange(product.rowIndex, 'code', e.target.value)} className={cn('min-w-[100px]', product.errorMessage?.includes('código') && 'border-destructive')} /></TableCell>}
                          {batchIndex === 0 && <TableCell rowSpan={product.batches.length} className="align-top pt-2"><Input value={product.name} onChange={e => handleFieldChange(product.rowIndex, 'name', e.target.value)} className={cn('min-w-[150px]', product.errorMessage?.includes('nombre') && 'border-destructive')} /></TableCell>}
                          ...
                          {batchIndex === 0 && (
                            <TableCell rowSpan={product.batches.length} className="align-top pt-2">
                              <Select value={product.currency} onValueChange={(value) => handleFieldChange(product.rowIndex, 'currency', value)}>
                          ...
                          <TableCell className="pt-2">
                            <Input 
                                type="number" 
                                value={batch.cost} 
                                onChange={e => handleBatchFieldChange(product.rowIndex, batchIndex, 'cost', Number(e.target.value))} 
                          ...
                          <TableCell className="pt-2">
                            <Input 
                                type="number" 
                                value={batch.price} 
                                onChange={e => handleBatchFieldChange(product.rowIndex, batchIndex, 'price', Number(e.target.value))} 
                          ...
                          <TableCell className="pt-2">
                            <Input 
                                type="number" 
                                value={batch.stock}
                                onChange={e => handleBatchFieldChange(product.rowIndex, batchIndex, 'stock', Number(e.target.value))} 
                                className="min-w-[80px]"
                            />
                          </TableCell>
                          
                          {batchIndex === 0 && <TableCell rowSpan={product.batches.length} className="align-top pt-2">
                            <Select value={existingCategories.find(c=>c.name === product.category)?.id || ''} onValueChange={(value) => handleFieldChange(product.rowIndex, 'category', existingCategories.find(c=>c.id === value)?.name || '')}>
