...
export const createClient = onCall(async (request: CallableRequest) => {
    if (!request.auth) {
...
export const updateClient = onCall(async (request: CallableRequest) => {
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "Debes estar autenticado.");
    }
...
export const deleteClient = onCall(async (request: CallableRequest) => {
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "Debes estar autenticado.");
    }
