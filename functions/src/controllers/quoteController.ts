...
export const createQuote = onCall({ cors: true, maxInstances: 1 }, async (request) => {
    if (!request.auth) {
...
export const updateQuote = onCall({ cors: true, maxInstances: 1 }, async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'User must be logged in.');
    }
    try {
...
export const deleteQuote = onCall({ cors: true, maxInstances: 1 }, async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'User must be logged in.');
    }
...
export const convertQuoteToInvoice = onCall({ cors: true, maxInstances: 1 }, async (request) => {
    if (!request.auth) {
        throw new HttpsError('unauthenticated', 'User must be logged in.');
    }
